...
#define S_DISTANCE_IN 7 //Back wheel distance in inches
...
/* Enumerations */
typedef enum _turnDir //Defining turn direction (turnDir)
{
	cw,
	ccw,
	ch
...
/* Structures (class without Functions) used to group varibles a data type that consits of more data */
typedef struct _pos
{
	float a; //Angle
	float y;
...
typedef struct _vel
{
	...
	float y;
	float x;
	unsigned long lstChecked; // How long ago it was checked (time) unsigned means you get an extra bit to store a number usally used in time
	float lstPosA;
	float lstPosY;
	float lstPosX;
